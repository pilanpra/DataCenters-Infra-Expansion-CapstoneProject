...
CREATE OR REPLACE TABLE CAPSTONE.STATE_DATA_CENTER.FLORIDA_FLAT (
...
SELECT 
    ...
    ESTABLISHED,
    DOWNLOADS,
    -- Convert CAPACITY JSON string into VARIANT before extracting fields
    PARSE_JSON(CAPACITY):"Fully Built-Out Power"::VARCHAR AS FULLY_BUILT_OUT_POWER,
    PARSE_JSON(CAPACITY):"Fully Built-Out Whitespace"::VARCHAR AS FULLY_BUILT_OUT_WHITESPACE,
    PARSE_JSON(CAPACITY):"Total Building Size"::VARCHAR AS TOTAL_BUILDING_SIZE,
    -- Similarly for SERVICES
    PARSE_JSON(SERVICES):"Full Cabinets"::VARCHAR AS FULL_CABINETS,
    PARSE_JSON(SERVICES):"Partial Cabinets"::VARCHAR AS PARTIAL_CABINETS,
    PARSE_JSON(SERVICES):"Shared Rackspace"::VARCHAR AS SHARED_RACKSPACE,
    PARSE_JSON(SERVICES):"Cages"::VARCHAR AS CAGES,
    PARSE_JSON(SERVICES):"Suites"::VARCHAR AS SUITES,
    PARSE_JSON(SERVICES):"Build-to-Suit"::VARCHAR AS BUILD_TO_SUIT,
    PARSE_JSON(SERVICES):"Footprints"::VARCHAR AS FOOTPRINTS,
    PARSE_JSON(SERVICES):"Remote Hands"::VARCHAR AS REMOTE_HANDS,
    -- And for POWER
    PARSE_JSON(POWER):"Max power/area"::VARCHAR AS MAX_POWER_AREA,
    PARSE_JSON(POWER):"UPS Redundancy"::VARCHAR AS UPS_REDUNDANCY,
    PARSE_JSON(POWER):"Cooling Redundancy"::VARCHAR AS COOLING_REDUNDANCY,
    PARSE_JSON(POWER):"Standby Power Redundancy"::VARCHAR AS STANDBY_POWER_REDUNDANCY,
    -- For COMPLIANCE
    PARSE_JSON(COMPLIANCE):"Tier Design"::VARCHAR AS TIER_DESIGN,
    PARSE_JSON(COMPLIANCE):"DSS PCI Certified"::VARCHAR AS DSS_PCI_CERTIFIED,
    PARSE_JSON(COMPLIANCE):"ISO27001 Certified"::VARCHAR AS ISO27001_CERTIFIED,
    PARSE_JSON(COMPLIANCE):"HIPAA"::VARCHAR AS HIPAA,
    PARSE_JSON(COMPLIANCE):"SOC 2 Type II Certified"::VARCHAR AS SOC_2_TYPE_II_CERTIFIED,
    PARSE_JSON(COMPLIANCE):"SOC 3 Type 2 Certified"::VARCHAR AS SOC_3_TYPE2_CERTIFIED,
    PARSE_JSON(COMPLIANCE):"NIST 800-53"::VARCHAR AS NIST_800_53,
    -- For SECURITY
    PARSE_JSON(SECURITY):"CCTV surveillance"::VARCHAR AS CCTV_SURVEILLANCE,
    PARSE_JSON(SECURITY):"Biometric Access Control"::VARCHAR AS BIOMETRIC_ACCESS_CONTROL,
    PARSE_JSON(SECURITY):"Card Access Control"::VARCHAR AS CARD_ACCESS_CONTROL,
    PARSE_JSON(SECURITY):"Onsite Security Staff"::VARCHAR AS ONSITE_SECURITY_STAFF,
    PARSE_JSON(SECURITY):"Onsite Technical Staff"::VARCHAR AS ONSITE_TECHNICAL_STAFF,
    -- For BUILDING
    PARSE_JSON(BUILDING):"Year Operational"::VARCHAR AS YEAR_OPERATIONAL,
    PARSE_JSON(BUILDING):"Roof Access"::VARCHAR AS ROOF_ACCESS,
    PARSE_JSON(BUILDING):"Building Floors"::VARCHAR AS BUILDING_FLOORS,
    PARSE_JSON(BUILDING):"Max Floor Load"::VARCHAR AS MAX_FLOOR_LOAD,
    -- For AMENITIES
    PARSE_JSON(AMENITIES):"Meet-Me-Room (MMR)"::VARCHAR AS MEET_ME_ROOM_MMR,
    -- For STATISTICS
    PARSE_JSON(STATISTICS):"Internet Exchange Points"::VARCHAR AS INTERNET_EXCHANGE_POINTS,
    PARSE_JSON(STATISTICS):"Peering networks"::VARCHAR AS PEERING_NETWORKS,
    PARSE_JSON(STATISTICS):"Network providers"::VARCHAR AS NETWORK_PROVIDERS,
    PARSE_JSON(STATISTICS):"Tenants offering colocation"::VARCHAR AS TENANTS_OFFERING_COLOCATION,
    PARSE_JSON(STATISTICS):"Child-Data Center listings"::VARCHAR AS CHILD_DATA_CENTER_LISTINGS
FROM CAPSTONE.STATE_DATA_CENTER.FLORIDA;




INSERT INTO CAPSTONE.STATE_DATA_CENTER.FLORIDA_FLAT
(
    LOCATION,
    ENERGY,
    AREA,
